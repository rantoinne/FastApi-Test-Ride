...
def read_post(
    ...
    post = _services.read_post(db=db, post_id=post_id)
    if post is None:
        raise _fastapi.HTTPException(
            status_code=404, detail="Post you are searching for doesn't exist"
        )
    return post
...
def delete_post(
    ...
    return {
        "message": f"Successfully deleted post with id: {post_id}"
    }
...
def update_post(
    ...
    post = _services.update_post(db=db, post=post, post_id=post_id)
    return post
